...
public class DataConverter {
    ...
    public static int parseStockValue(String label) {
        ...
        Assert.assertTrue(qtyMatcher.find(), "Unable to extract In Stock (quantity) value1!");
        return Integer.parseInt(qtyMatcher.group(1));
    }

    public static float parsePriceValue2(String label) {
        Matcher qtyMatcher = Pattern.compile("(\\d+,\\d+)").matcher(label);
        Assert.assertTrue(qtyMatcher.find(), "Unable to extract In Stock (quantity) value2!");
        return Float.parseFloat(qtyMatcher.group(1).replace(',', '.'));
    }
    ...
    public static float parsePriceValue(String label) {
        ...
        Assert.assertTrue(priceMatcher.find(), "Unable to extract price value3!");
